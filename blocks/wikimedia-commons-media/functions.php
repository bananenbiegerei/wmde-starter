...
function bbwkc_get_media_query($file, $lang = 'en')
{
	...
	$mime_type = bbwkc_get_media_type($file);
	$hash = md5($file);
	...
	switch ($type) {
		case 'image':
			...
			$out['srcset'] = bbkwc_img_srcset($file, bbwkc_get_wp_image_sizes(), $media['imageinfo'][0]['width'], $h);
			break;
		case 'video':
			$out['image'] = bbwkc_get_video_poster($file, $h);
			$out['media_url'] = $media['imageinfo'][0]['url'];
			if ($mime_type == 'video/ogg') {
				$out['media_url'] = bbwkc_get_video_alt($file, $h);
			}
			break;
		case 'audio':
			$out['media_url'] = $media['imageinfo'][0]['url'];
			if ($mime_type == 'application/ogg') {
				$out['media_url'] = bbwkc_get_audio_alt($file, $h);
			}
			break;
		case 'pdf':
			$out['image'] = $media['imageinfo'][0]['url'];
