...
<div class="bg-neutral min-h-[12rem]">
    <div class="container grid grid-cols-12">
        <div class="col-span-12 lg:col-span-10 lg:col-start-2">
            ...
            <?php if (has_excerpt()): ?>
            <div class="text-xl lg:text-2xl mb-10 font-normal">
      ...
      $link = get_sub_field('link') ? get_sub_field('link') : ['title' => 'Missing Link!', 'url' => '#', 'target' => '_self'];
        ...
        ?>
            <div class="flex">
                <a class="btn btn-<?= $color ?> <?= $icon ? '' : '' ?>" href="<?= esc_url($link['url']) ?>" target="<?= esc_attr($link['target']) ?>">
                    <?= esc_html($link['title']) ?>
                        </a>
                    </div>
                    <?php
...
<div class="pb-10 rounded-b-3xl">
    <div class="container grid grid-cols-12">
        <div class="col-span-12 pt-5">
            ...
            <h1><?php the_title(); ?></h1>
            <?php if (has_excerpt()): ?>
            <div class="text-xl lg:text-2xl font-normal mb-5">
                <?php the_excerpt(); ?>
            </div>
            <?php endif; ?>
            <?php if (have_rows('call_to_actions_in_header')): ?>
